...
fn::centos::install_docker()
{
  ...
  if fn::package_exists docker-engine-selinux-${version}*; then exit 1; fi

  # 安装指定版本（docker-engine-selinux 是 docker-engine 的依赖包，所以先安装并指定相同的版本，否则会安装最新版本）
  fn::install_package docker-engine-selinux-${version}* docker-engine-${version}*
}
