...
fn::install_ethereum_mist()
{
  local version=${1:-$MIST_VERSION}
  local version2=$(echo $version | sed -e "s|\.|-|g")

  fn::install_package unzip

  if ! fn::command_exists mist || [ -z "$(mist --version | grep $version)" ]; then
    rm -rf /tmp/mist* && mkdir -p {/tmp/mist,/usr/local/mist}
    wget -O /tmp/mist.tar.gz https://github.com/ethereum/mist/releases/download/v${version}/Mist-linux64-${version2}.zip
    unzip /tmp/mist.tar.gz -d /tmp/mist
    cp -rf /tmp/mist/linux-unpacked/* /usr/local/mist/
    rm -rf /tmp/mist*

    rm -f /usr/bin/mist && ln -s /usr/local/mist/mist /usr/bin/mist
  fi
}
