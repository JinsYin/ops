...
fn::install_jx()
{
  local version=${1:-$JX_VERSION}

  fn::install_package wget

  if ! fn::command_exists jx; then
    rm -rf /tmp/jenkins-x* && mkdir -p /tmp/jenkins-x
    wget -O /tmp/jenkins-x/jx.tar.gz https://github.com/jenkins-x/jx/releases/download/v${version}/jx-linux-amd64.tar.gz
    tar -zxf /tmp/jenkins-x/jx.tar.gz -C /tmp/jenkins-x
    mv /tmp/jenkins-x/jx /usr/local/bin
